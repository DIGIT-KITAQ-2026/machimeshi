...
## 複数人で開発する場合（2人目以降のメンバー向け）

チームの誰か1人が下記の手順1〜4で既にSupabaseプロジェクトを作成済みなら、
2人目以降のメンバーは**手順1〜4をやり直す必要はありません**。以下だけでOKです。

1. プロジェクト作成者に、Supabaseダッシュボードの
   **Project Settings > Team**（組織のメンバー管理画面）から自分のメールアドレスを
   招待してもらう
2. 招待メールから参加し、自分のSupabaseアカウントでログインする
3. 参加したプロジェクトの **Settings > API** を自分で開き、
   **Project URL** と **anon public** キーを自分で取得する
   （キーをチャット等でそのまま送ってもらう必要はない）
4. `cp .env.example .env` して、取得した値を書き込む（手順5と同じ）

> 補足: `VITE_SUPABASE_ANON_KEY`はRLS（Row Level Security）で保護される前提の
> 公開用キーであり、本当に守るべき`service_role`キー（本プロジェクトでは未使用）とは
> 性質が異なる。とはいえ`.env`自体はプロジェクトごとに異なる設定ファイルなので、
> 各自の環境で`.env.example`からコピーして使い、gitにはコミットしない。

新しいテーブルやRLSポリシーが`supabase/schema.sql`に追加された場合は、
プロジェクトのオーナー（またはSQL Editorを使える権限を持つメンバー）が
SQL Editorでファイル全体を再実行すれば、チーム全員に反映される
（メンバー各自が実行し直す必要はない。同じプロジェクト＝同じDBを見ているため）。
